...
import { addMoneySource } from "./user.constants";

const formSchema = z.object({
  amount: z.string().min(1, { message: "Amount is required" }),
  addMoneySource: z.enum(addMoneySource),
});
...
export function AddMoney() {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      amount: "",
      addMoneySource: addMoneySource.BANK,
    },
  });
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    try {
      ...
      await addMoney({ ...data, amount }).unwrap();
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent>
                            {Object.values(addMoneySource).map((source) => (
