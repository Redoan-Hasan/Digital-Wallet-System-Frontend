...
import { useCashOutMoneyMutation } from "@/redux/features/wallet/wallet.api";
...
export function CashOut() {
  const [cashOutMoney, { isLoading }] = useCashOutMoneyMutation();
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    try {
      ...
      await cashOutMoney({ ...data, amount }).unwrap();
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Agents VaultPay Number</FormLabel>
                        <FormControl>
                          <Input
                            type="text"
                            placeholder="Enter Agent's VaultPay number"
